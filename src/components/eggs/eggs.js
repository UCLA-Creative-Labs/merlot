...
class EggComponent extends React.Component {
    ...
    handleSubmit = (event) => {
        ...
        //console.log(this.egg.password)
        console.log(this.state.value)
        let userInput = String(this.state.value).toLowerCase().replace(/\s+/g, ''); //Remove spaces and make lowercase
        if ( userInput != this.egg.password){
            alert('Nope try again. Hint: ' + this.egg.hint);
        }
        else{
            navigate("http://localhost:8000/eggs/"+this.egg.next)
        }
    ...
    render () {
        ...
        return(
            ...
            >
                ...
                <center>
                    <img src={this.egg.img} style={{width:"25%", height:"25%"}}/>
                    <br/>
